...
class Simulation:
 def __init__(self):
  self.t=0.; self.production=0.; self.alarm=False; self.paused=False; self.speed=1.; self.machines=[]
 ...
 def snapshot(self):
  methane=.55+.18*math.sin(self.t/7)+random.uniform(-.04,.04)+(1.8 if self.alarm else 0); temp=29+2*math.sin(self.t/13)+random.uniform(-.5,.5); water=max(0,18+8*math.sin(self.t/17)+random.uniform(-2,2)); vent=max(55,92+8*math.sin(self.t/11))
  ...
  return {'time':round(self.t,1),'production':round(self.production,2),'alarm':self.alarm,'paused':self.paused,'metrics':{'methane':round(methane,2),'temperature':round(temp,1),'water':round(water,1),'ventilation':round(vent,1),'active_machines':active,'total_machines':len(self.machines)},'machines':self.machines}
sim=Simulation()
@app.get('/')
def index():return FileResponse(ROOT/'frontend'/'index.html')
@app.post('/api/alarm/{state}')
def alarm(state:str):sim.alarm=state.lower() in ('on','true','1');sim.log('alarm','Emergency '+('activated' if sim.alarm else 'cleared'));return {'alarm':sim.alarm}
